...
\`/remove_keyword <правило>\` - Удалить конкретное правило.
\`/clean_keywords\` - Удалить ВСЕ правила.
\`/list_keywords\` - Показать список ключевых слов и правил.
\`/add_source <ID_группы>\` - Добавить группу для мониторинга.
\`/remove_source <ID_группы>\` - Удалить конкретную группу.
\`/clean_sources\` - Удалить ВСЕ группы.
\`/list_sources\` - Показать список групп.
\`/set_target <ID_канала>\` - Установить канал для уведомлений.
\`/get_target\` - Показать текущий канал для уведомлений.
\`/remove_target\` - Удалить канал для уведомлений (остановить алерты).
